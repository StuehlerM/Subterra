...
- Art is loaded from **`public/assets/`** (PNG per tile/entity) via the **AssetRegistry**;
  replace a file (same name) + reload to see it. Missing files fall back to flat colours
  (tiles) / emoji (entities). Placeholders are generated by `scripts/gen-placeholders.mjs`.
